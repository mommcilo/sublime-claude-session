...
2. Copy or symlink this directory into that folder as `ClaudeProxy`:
   ```
   ln -s /Users/momcilo/Development/PytonProject/SublimeClaudeProxy \
         "$HOME/Library/Application Support/Sublime Text/Packages/ClaudeProxy"
   ```
3. Restart Sublime Text.

## Usage

1. Open the Command Palette (`Ctrl/Cmd+Shift+P`) and run **ClaudeProxy: Start Session**. The plugin creates `ClaudeSession-<uuid>.txt` in the session's working directory and opens it as a regular text file with a banner and a `> ` prompt marker.
...
|---|---|---|
| ClaudeProxy: Start Session | `claude_proxy_start` | Opens a new session view. |
| ClaudeProxy: Send Prompt | `claude_proxy_send` | Sends text after the current prompt marker. Bound to `Ctrl+Enter` in session views. |
| ClaudeProxy: Cancel Request | `claude_proxy_cancel` | Terminates the in-flight `claude` process; keeps the session. |
| ClaudeProxy: Stop Session | `claude_proxy_stop` | Ends the session and discards state. |

## Key bindings

`Ctrl+Enter` is bound to send only inside a ClaudeProxy session view (gated by the `claude_proxy_active` view setting), so it won't shadow your global binding elsewhere.

## Settings

`Preferences > Package Settings > ClaudeProxy > Settings`:
...
Each session generates a UUID and invokes `claude --print` once with `--session-id <uuid>` for the first turn, then `--resume <uuid>` for every subsequent turn. Output is read from the subprocess on a background thread and appended to the view via `sublime.set_timeout`. The buffer position right after the last `> ` marker is the *prompt anchor* — `claude_proxy_send` sends everything between the anchor and end-of-buffer.

## Troubleshooting

- **"could not find `claude`"** — install the CLI or set an absolute path in `claude_command`.
- **No response / hung session** — run *ClaudeProxy: Cancel Request*, then check Sublime's console (`` Ctrl+` ``) for the `[ClaudeProxy] cwd=… args=…` log line and any subprocess error.
