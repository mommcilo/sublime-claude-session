...
A Sublime Text 4 plugin (`ClaudeProxy`) that embeds an interactive Claude Code CLI session inside a regular Sublime text-file buffer. On *Start Session* the plugin pre-writes `<cwd>/ClaudeSession-<uuid>.txt` (banner + initial `> ` marker), opens that file in a new tab, and routes streamed CLI output into it. The buffer is NOT scratch — user can save (`Ctrl/Cmd+S`) and edit freely; closing a dirty buffer triggers the normal save prompt.

Shells out to `claude --print` with session-id continuity (first turn `--session-id <uuid>`, subsequent turns `--resume <uuid>`).

Not a package distributed via Package Control yet — installed by symlinking this dir into `Packages/ClaudeProxy`.

## Layout

- `claude_proxy.py` — the entire plugin. Single file. Contains:
  - `SESSIONS` — module-level `{view_id: ClaudeSession}` map; the source of truth for which views are session views.
  - `format_banner(session_id, cwd)` — module-level; produces the banner text that's pre-written to disk by `ClaudeProxyStartCommand` (so the file is non-empty before the view opens).
  ...
  - `ClaudeProxyAppendCommand` (TextCommand) — the only path that writes to the buffer; everything else hops through `sublime.set_timeout` → `view.run_command("claude_proxy_append", ...)`.
  - `ClaudeProxyStartCommand` / `SendCommand` / `CancelCommand` / `StopCommand` — palette entry points. Start command pre-writes `<cwd>/ClaudeSession-<uuid>.txt`, opens it via `window.open_file(...)`, then polls `view.is_loading()` to set `prompt_anchor = view.size()` once the file finishes loading.
  - `ClaudeProxyListener` — `on_close` tears the session down (subprocess only; the `.txt` file persists on disk).
- `Default.sublime-commands` — palette captions.
- `Default.sublime-keymap` — `Ctrl+Enter` → `claude_proxy_send`, scoped to views with `claude_proxy_active=true`.
- `ClaudeProxy.sublime-settings` — exposes `claude_command` (argv list).
- `README.md` — user-facing install + usage docs.

## Key invariants / things easy to break

- **Buffer writes must go through `claude_proxy_append`.** Sublime requires an `edit` token; calling `view.insert` from a worker thread will crash. `_append` already routes correctly — preserve that.
...
- **`prompt_anchor` is updated after every append.** It marks the start of the user's next prompt. `claude_proxy_send` reads `[prompt_anchor, view.size())` as the prompt. If you add new ways to write to the buffer, make sure the anchor advances afterward or sends will re-submit old output.
...
1. Symlink the repo into `Packages/ClaudeProxy` (one-time).
2. Reload the plugin: edit & save `claude_proxy.py`, or `Tools > Developer > Reload Plugin`.
3. Open Sublime console (`` Ctrl+` ``) to watch the `[ClaudeProxy]` log lines.
4. Run *ClaudeProxy: Start Session*, send a prompt, watch it stream back.
